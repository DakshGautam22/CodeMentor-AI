...
export const EditorPanel: React.FC = () => {
  const { code, language, theme, isOpened, setCode, setLanguage, setIsOpened } = useEditorStore();
    ...
    >
      ...
      <div
        ...
        className={`w-1.5 h-full cursor-col-resize hover:bg-brand-500/50 hover:w-2 active:bg-brand-500 transition-all absolute left-0 top-0 z-50 ${
          isResizing ? "bg-brand-500" : "bg-dark-800"
        }`}
      />

      <div className="flex-1 h-full glass-panel flex flex-col pl-1.5">
        {/* Editor panel header controls */}
        <div className="p-3 border-b border-dark-800 flex items-center justify-between bg-dark-950/40">
          <div className="flex items-center gap-2">
            <Code className="w-4 h-4 text-brand-400" />
            <select
              value={language}
              onChange={(e) => setLanguage(e.target.value)}
              className="bg-dark-900 border border-dark-800 text-xs rounded-lg px-2.5 py-1 text-slate-200 focus:outline-none focus:border-brand-500"
            >
          ...
          <div className="flex items-center gap-1.5">
            <button
              onClick={formatCode}
              className="p-1.5 hover:bg-white/5 border border-dark-800 hover:border-dark-700 rounded-lg text-slate-400 hover:text-slate-200 text-xs flex items-center gap-1"
              title="Auto Format"
            >
            ...
            <button
              onClick={handleCopy}
              className="p-1.5 hover:bg-white/5 border border-dark-800 hover:border-dark-700 rounded-lg text-slate-400 hover:text-slate-200"
              title="Copy Code"
            >
        ...
        <div className="flex-1 w-full overflow-hidden bg-[#1e1e1e]">
          <Editor
            ...
            loading={
              <div className="flex flex-col items-center justify-center h-full gap-2 text-slate-400 text-xs font-medium">
